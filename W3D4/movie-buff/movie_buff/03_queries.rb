...
def golden_age
  ...
  Movie.select("FLOOR(yr / 10) AS decade")
    .group("FLOOR(yr / 10)")
    .order("AVG(score) DESC")
    .limit(1).first.decade.to_i * 10
end
...
def costars(name)
  ...
  Actor.find_by(name: name).movies
    .joins(:actors)
    .where("name != ?", name).pluck("actors.name")
end
...
def starring(whazzername)
  ...
  matcher = "%#{whazzername.delete(" ").chars.join("%")}%"
  Movie
    .joins(:actors)
    .where("LOWER(actors.name) LIKE ?", matcher)
end
